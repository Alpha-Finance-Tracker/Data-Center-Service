...
async def add_food_into_db(name,price,category,type,date,token):
    event_date = datetime.strptime(date, '%d.%m.%Y').date()
    update_query('INSERT INTO expenditures(name,price,category,food_type,date,user_id) VALUES(%s,%s,%s,%s,%s,%s)',(name,price,category,type,event_date, token.get("user_id")))
    return "Product added successfully "

async def get_food_expenditures_from_db(start_date,end_date):
