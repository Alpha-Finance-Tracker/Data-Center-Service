...
health_expenditures_router = APIRouter(prefix='transportation')


@health_expenditures_router.put('/expenditure')
async def add_health(token,name:str = Query(...,),price:float = Query(...,),type=Query(),date=Query()):
    await verify_token(token)
    print('To do')
@health_expenditures_router.get('/monthly_expenditures')
...
@health_expenditures_router.get('/biggest_passives')
