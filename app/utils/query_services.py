...
async def add_product_into_db(name,price,type,date):
    event_date = datetime.strptime(date, '%d.%m.%Y').date()
    update_query('INSERT INTO expenditures(name,price,date,user_id) VALUES(%s,%s,%s,%s,%s)',(name,price,type,event_date, 8))


async def get_expenditures_from_db(month):
    products =  read_query('SELECT name,price FROM expenditures WHERE MONTH(date) = %s', (month,))
    total_spent = read_query('SELECT SUM(price) FROM expenditures WHERE MONTH(date) = %s',(month,) )
